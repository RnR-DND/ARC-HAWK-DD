...
func (s *ReportService) generateContent(ctx context.Context, job reportJob) ([]byte, string, error) {
	...
	case domain.ReportPDF:
		return s.renderPDF(snap, facts, hotspots)
	default:
...
func (s *ReportService) renderPDF(snap *domain.Snapshot, facts []*domain.SnapshotFact, hotspots []*domain.RiskHotspot) ([]byte, string, error) {
	...
	pdf.CellFormat(0, 10, "Board Data Risk Report", "", 1, "L", false, 0, "")
	pdf.SetFont("Helvetica", "", 10)
	pdf.SetTextColor(100, 100, 100)
	pdf.CellFormat(0, 6, "Snapshot: "+snap.TakenAt.Format("2006-01-02 15:04 UTC"), "", 1, "L", false, 0, "")
	pdf.SetTextColor(26, 26, 26)
	pdf.Ln(4)
	...
	pdf.SetFont("Helvetica", "", 10)
	totalAssets, totalFindings := 0, 0
	for _, f := range facts {
		totalAssets += f.AssetCount
		totalFindings += f.FindingCount
	}
	pdf.CellFormat(60, 6, fmt.Sprintf("Total Assets Scanned: %d", totalAssets), "", 0, "L", false, 0, "")
	pdf.CellFormat(60, 6, fmt.Sprintf("Total PII Findings: %d", totalFindings), "", 0, "L", false, 0, "")
	pdf.CellFormat(60, 6, fmt.Sprintf("Data Sources: %d", len(facts)), "", 1, "L", false, 0, "")
	pdf.Ln(4)

	// Source breakdown table
	if len(facts) > 0 {
		pdf.SetFont("Helvetica", "B", 12)
		pdf.CellFormat(0, 8, "Source Breakdown", "", 1, "L", false, 0, "")
		pdf.SetFillColor(245, 245, 245)
		pdf.SetFont("Helvetica", "B", 9)
		pdf.CellFormat(55, 6, "Source", "1", 0, "L", true, 0, "")
		pdf.CellFormat(35, 6, "Classification", "1", 0, "L", true, 0, "")
		pdf.CellFormat(25, 6, "Assets", "1", 0, "C", true, 0, "")
		pdf.CellFormat(25, 6, "Findings", "1", 0, "C", true, 0, "")
		pdf.CellFormat(35, 6, "Sensitivity", "1", 1, "C", true, 0, "")
		...
		for i, f := range facts {
			...
			pdf.CellFormat(55, 5, f.SourceName, "1", 0, "L", fill, 0, "")
			pdf.CellFormat(35, 5, f.Classification, "1", 0, "L", fill, 0, "")
			pdf.CellFormat(25, 5, strconv.Itoa(f.AssetCount), "1", 0, "C", fill, 0, "")
			pdf.CellFormat(25, 5, strconv.Itoa(f.FindingCount), "1", 0, "C", fill, 0, "")
			pdf.CellFormat(35, 5, fmt.Sprintf("%.2f", f.SensitivityAvg), "1", 1, "C", fill, 0, "")
		}
		pdf.Ln(4)
	}
	...
	if len(hotspots) > 0 {
		pdf.SetFont("Helvetica", "B", 12)
		pdf.CellFormat(0, 8, "Top Risk Hotspots", "", 1, "L", false, 0, "")
		pdf.SetFillColor(245, 245, 245)
		pdf.SetFont("Helvetica", "B", 9)
		pdf.CellFormat(95, 6, "Asset", "1", 0, "L", true, 0, "")
		pdf.CellFormat(30, 6, "Classification", "1", 0, "L", true, 0, "")
		pdf.CellFormat(30, 6, "Risk Score", "1", 0, "C", true, 0, "")
		pdf.CellFormat(20, 6, "Findings", "1", 1, "C", true, 0, "")
		pdf.SetFont("Helvetica", "", 9)
		for i, h := range hotspots {
			...
			pdf.CellFormat(95, 5, h.AssetName, "1", 0, "L", fill, 0, "")
			pdf.CellFormat(30, 5, h.Classification, "1", 0, "L", fill, 0, "")
			pdf.CellFormat(30, 5, fmt.Sprintf("%.1f", h.Score), "1", 0, "C", fill, 0, "")
			pdf.CellFormat(20, 5, strconv.Itoa(h.FindingCount), "1", 1, "C", fill, 0, "")
		}
	}
	...
	pdf.CellFormat(0, 6, "Generated by ARC-HAWK-DD · "+time.Now().UTC().Format(time.RFC3339), "", 1, "C", false, 0, "")
