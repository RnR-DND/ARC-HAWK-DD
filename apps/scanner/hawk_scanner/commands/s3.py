...
def connect_s3(args, access_key, secret_key, bucket_name):
    try:
        ...
        system.print_error(
            args, f"[bold red]Failed[/bold red] to connect to S3 bucket: {e}")
...
def execute(args):
    results = []
    shouldDownload = True
    system.print_info(args, "Running Checks for S3 Sources")
    ...
    if 'sources' in connections:
        ...
        if s3_config:
            for key, config in s3_config.items():
                ...
                system.print_info(
                    args, f"Checking S3 profile: '{key}' with bucket '{bucket_name}'")
                if access_key and secret_key and bucket_name:
                    bucket = connect_s3(
                        args, access_key, secret_key, bucket_name)
                    if bucket:
                        for obj in bucket.objects.all():
                            if quick_exit and len(results) >= max_matches:
                                system.print_info(
                                    args, f"Quick exit: Found {max_matches} matches, exiting...")
                                break
                            ...
                            if config.get("cache"):
                                if os.path.exists(file_path):
                                    ...
                                    if remote_etag != local_etag:
                                        system.print_debug(
                                            args, "File in S3 bucket has changed, downloading it again...")
                                        shouldDownload = True
                                    else:
                                        shouldDownload = False
                        ...
                        system.print_error(
                            args, f"Failed to connect to S3 bucket: {bucket_name}")
                else:
                    system.print_error(
                        args, f"Incomplete S3 configuration for key: {key}")
            if config.get("cache") is False:
                os.system("rm -rf data/s3")
        else:
            system.print_error(
                args, "No S3 connection details found in connection.yml")
    else:
        system.print_error(
