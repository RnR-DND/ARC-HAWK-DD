...
def connect_google_cloud(args, bucket_name, credentials_file):
    try:
        ...
        system.print_debug(
            args, f"Connected to Google Cloud Storage bucket: {bucket_name}")
        return bucket
    ...
    if 'sources' in connections:
        ...
        if gcs_config:
            for key, config in gcs_config.items():
                ...
                if bucket_name:
                    bucket = connect_google_cloud(
                        args, bucket_name, credentials_file)
                    if bucket:
                            ...
                            if config.get("cache"):
                                if os.path.exists(file_path):
                                    ...
                                    if remote_etag != local_etag:
                                        system.print_debug(
                                            args, "File in Google Cloud Storage bucket has changed, downloading it again...")
                                        shouldDownload = True
                                    else:
                                        shouldDownload = False
                        ...
                        system.print_error(
                            args, f"Failed to connect to Google Cloud Storage bucket: {bucket_name}")
                else:
                    system.print_error(
                        args, f"Incomplete Google Cloud Storage configuration for key: {key}")
        else:
            system.print_error(
                args, "No Google Cloud Storage connection details found in connection.yml")
    else:
        system.print_error(
